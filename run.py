...
from models import build_rf, build_simply
...
logger.debug('Using RandomForestRegressor')
est = RandomForestRegressor()
est = build_simply(x, y, est)

logger.debug('Using ExtraTreesRegressor')
est = ExtraTreesRegressor()
est = build_simply(x, y, est)

logger.debug('Using LinearRegression')
est = LinearRegression()
est = build_simply(x, y, est)

logger.debug('Using BayesianRidge')
est = BayesianRidge()
est = build_simply(x, y, est)

logger.debug('Using SVR and rbf')
est = SVR()
est = build_simply(x, y, est)
...
# logger.info('Mean of submit gives %0.5f' %np.mean(np.mean(sub.iloc[:, 1:])))
